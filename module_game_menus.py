...
#This needs to be the first window!!!
 ...
 ^^Option 1: Add bandit heroes ({s1})^If enabled, 6 bandit heroes will appear in the wildness according to your character's level.\
 The bandit parties will come to accompany them. They become much united, not like a sheet of loose sand.\
 ^^Option 2: Party Vigor ({s2})^If enabled, party vigor will be a new factor for player's party morale.\
 Party vigor will be consumed when marching and be recovered when resting. It's more realistic.",
    "none", 
    [
      (try_begin),
    ...
    [
      ...
      ("option_2", 
        ...
        "{s2} the Option 2.",
        [
          (val_add, "$g_enable_party_vigor", 1),
          (val_mod, "$g_enable_party_vigor", 2),
          (jump_to_menu,"mnu_start_game_options"),
        ]),
      ("continue", [],
        "Continue...",
...
#         (change_screen_return),
      ...
      ("tutorial_cheat",[(eq,1,0)],"{!}CHEAT!",
       [
         ...
         #(troop_raise_skill, "trp_player", skl_prisoner_management, 5), ## skill overhaul
...
##  ),
  ...
  (
    ...
    "Tips: Clicking on a thumbnail displays the full image.^^\
 Switching to next image: Right Arrow key/Down Arrow key/Page Down key^\
 Switching to previous image: Left Arrow key/Up Arrow key/Page Up key^\
 Exit the full image mode: Space key/Enter key/Escape key/Backspace key", ## CC
    "none",
    [
      (try_begin),
...
##      ]),
      ...
      [
        ## CC
        (call_script, "script_init_gallery_data"),
  ...
  ("reports",0,
   "Character Renown: {reg15}, Honor Rating: {reg6}^Party Morale: {reg8}{reg11?, Party Vigor: {reg10}:}^Party Size Limit: {reg7}, Party Prisoner Size Limit: {reg12}^^{reg9?Next pay time:^{s1}:}^{s5}", ## CC
   "none",
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    (assign, ":party_prisoner_limit", reg0),
    (assign, reg12, ":party_prisoner_limit"),
    (assign, reg10, "$g_party_vigor"),
    (assign, reg11, "$g_enable_party_vigor"),
    ...
    [
      ("continue",[],"Continue...",
        [
           ## CC
           # attributes
           (try_for_range, ":attribute_slot", cb_attribute_slots_begin, cb_attribute_slots_begin + 4),
             (troop_get_slot, ":attribute_value", "trp_cb_troop_total", ":attribute_slot"),
             (val_sub, ":attribute_value", 4),
             (gt, ":attribute_value", 0),
             (store_sub, ":attribute_id", ":attribute_slot", cb_attribute_slots_begin),
             (troop_raise_attribute, "trp_player", ":attribute_id", ":attribute_value"),
           (try_end),
           # skills
           (try_for_range, ":skill_slot", cb_skill_slots_begin, cb_skill_slots_begin + 42),
             (troop_get_slot, ":skill_value", "trp_cb_troop_total", ":skill_slot"),
             (gt, ":skill_value", 0),
             (store_sub, ":skill_id", ":skill_slot", cb_skill_slots_begin),
             (troop_raise_skill, "trp_player", ":skill_id", ":skill_value"),
           (try_end),
           # proficiency 
           (try_for_range, ":proficiency_slot", cb_proficiency_slots_begin, cb_proficiency_slots_begin + 7),
             (troop_get_slot, ":proficiency_value", "trp_cb_troop_total", ":proficiency_slot"),
             (gt, ":proficiency_value", 0),
             (store_sub, ":proficiency_id", ":proficiency_slot", cb_proficiency_slots_begin),
             (troop_raise_proficiency, "trp_player", ":proficiency_id", ":proficiency_value"),
           (try_end),
           
           # special skills which hidden
           (troop_raise_skill, "trp_player", skl_prisoner_management, 1),
           
           # renown
           (troop_get_slot, ":troop_renown", "trp_cb_troop_total", cb_troop_renown_slot),
           (troop_set_slot, "trp_player", slot_troop_renown, ":troop_renown"),
           # gold
           (store_troop_gold, ":troop_gold", "trp_cb_troop_total"),
           (try_begin),
             (eq, "$g_gather_npcs", 1),
             (val_add, ":troop_gold", 3300),
           (try_end),
           (try_begin),
             (gt, ":troop_gold", 0),
             (troop_add_gold, "trp_player", ":troop_gold"),
           (try_end),
           # inventory
           (troop_get_inventory_capacity, ":inv_cap", "trp_cb_troop_total"),
           (try_for_range, ":i_slot", 10, ":inv_cap"),
             (troop_get_inventory_slot, ":item", "trp_cb_troop_total", ":i_slot"),
             (troop_get_inventory_slot_modifier, ":imod", "trp_cb_troop_total", ":i_slot"),
             (gt, ":item", -1),
             (troop_add_item, "trp_player", ":item", ":imod"),
             (troop_set_inventory_slot, "trp_cb_troop_total", ":i_slot", -1),
           (try_end),
           # book for student
           (try_begin),
             ...
             (try_begin),
               (call_script, "script_cf_troop_has_item", "trp_tavern_bookseller_1", ":book_no"),
               (troop_remove_item, "trp_tavern_bookseller_1", ":book_no"),
             (else_try),
               (call_script, "script_cf_troop_has_item", "trp_tavern_bookseller_2", ":book_no"),
               (troop_remove_item, "trp_tavern_bookseller_2", ":book_no"),
             (try_end),
           (try_end),
           ## CC
           (try_begin),
...
## CC
  ...
  ("character_report",0,
   ...
   ## CC courtship_relations
	...
	("continue",[(eq,"$cheat_mode",1)],"{!}CHEAT! - increase persuasion",
       [
	   (troop_raise_skill, "trp_player", "skl_speechcraft", 1),
  ...
  ("party_size_report",0,
   ...
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    (assign, reg3, ":renown"),
    (str_store_string, s1, "@Current party size limit is {reg5}.^Current party size modifiers are:^^Base size:  +30^Player's level: +{reg7}^Leadership: {s2}{reg1}^Renown: {s4}{reg3}^Castles: {s6}{reg6}^TOTAL:  {reg5}"), ## CC
    ],
    [
      ("continue",[],"Continue...",
       [(jump_to_menu, "mnu_reports"),
  ...
  ("camp",mnf_scale_picture,
   ...
   [
    ...
    # [
      ...
      # (start_presentation, "prsnt_gallery"),
     # ]),
    ("camp_mod_opition",[],"MOD options.", [(start_presentation, "prsnt_mod_options")]),
     ...
     [
      ...
      (else_try),
        ...
        (try_for_range, ":cur_stack", 0, ":num_stacks"),
          ...
          (assign, ":troop_morale", 0),
          (store_troop_faction, ":faction_no", ":cur_troop_id"),
          (try_begin),
          ...
          (try_end),
          (val_add, ":recruit_chance", ":troop_morale"),
    ...
    ("camp_manage_inventory",
      ...
      ## CC
        ...
        (try_for_range, ":i_stack", 0, ":num_stacks"),
          ...
          (troop_set_auto_equip, ":this_hero", 0),
  ...
  ("cheat_find_item",0,
   ...
   [
     ...
     ## CC
     (store_item_value, ":num_items", "itm_no_item"),
     #(val_min, reg6, "itm_items_end"),
     (val_min, reg6, ":num_items"),
     ## CC
     (val_sub, reg6, 1),
     ],
    [
      ("cheat_find_item_next_range",[], "{!}Move to next item range.",
        [
          ...
          (try_begin),
            ...
            (store_item_value, ":num_items", "itm_no_item"),
            (ge, "$cheat_find_item_range_begin", ":num_items"),
            ## CC
...
#### export/import NPCs end ####
   ...
   [],
    ...
    [
      ("action_read_book_"+str(x+1),
        ...
        ]) for x in range(0, itm_book_wound_treatment_reference - itm_book_speechcraft)
    ]+[
 ...
 all your money, and ride off into the sunset....",
  ...
  (
    "simple_encounter",mnf_scale_picture|mnf_enable_hot_keys,
        ...
        (try_begin),
          ...
          (try_begin),
            ...
            (try_begin),
              ...
              # (assign, "$g_random_scene_size", 0), ## CC
            (try_end),
          (try_end),
          ...
          (try_begin),
            ...
            (else_try),
              ...
              (try_begin),
                ...
                (store_add, ":dest_string", "str_battlefield_small", "$g_random_scene_size_forests"),
                (str_store_string, s3, ":dest_string"),
                (str_store_string, s3, "@{s3} in a forest"),
              (else_try),
                (store_add, ":dest_string", "str_battlefield_small", "$g_random_scene_size"),
                (str_store_string, s3, ":dest_string"),
                (str_store_string, s3, "@{s3} in wildness"),
              (try_end),
            (try_end),
            ## CC
...
## CC
      ...
      ("choose_battle_options", [], "Battle Options.",
        [(start_presentation, "prsnt_battle_options"),]),
...
## CC
      ...
      [
        ...
        (display_message, "@Battle Advantage = {reg0}.", 0xFFFFFFFF), ## CC
  ...
  (
    ...
    [
        ...
        (else_try),                 
          ...
          ## CC 
          # move outlaws from companions to prisoners
  ...
  (
    ...
    [
      ...
      (try_for_range, ":unused", 0, ":end_cond"),
        ...
        ## skill overhaul
          # (else_try),
           # (eq, ":random_attribute", ca_vitality),
           # (str_store_string, s0, "@After the battle you find that your torso have been seriously injured. Your vitality is reduced by 1."),
        ## skill overhaul
          (else_try),
##            (eq, ":random_attribute", ca_intelligence),
  ...
  (
    ...
    "You are helping the {s2} against the {s1}. The battle is taking place in {s3}. You have {reg10} troops fit for battle against the enemy's {reg11}.^^P, C , I keys are enabled here.",
    "none",
    [                
    ...
    [
      ...
      ("join_siege_with_allies",[(neg|troop_is_wounded, "$g_player_troop")], "Join the next assault.", ## CC
       [
           ...
           (call_script, "script_calculate_battle_advantage"),
           ## CC
           # scale down the advantage a bit in sieges.
           # -2 for castle, -3 for town
           (val_sub, reg0, 2),
           (try_begin),
             (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
             (val_sub, reg0, 1),
...
#          (str_store_string, s3, "str_place_is_occupied_by_friendly"),
        ...
        (else_try),
          ...
          (main_party_has_troop, "trp_kidnapped_girl"), ## CC
          (call_script, "script_get_meeting_scene"), (assign, ":meeting_scene", reg0),
          (modify_visitors_at_site,":meeting_scene"),
          (reset_visitors),
          (set_visitor,0, "trp_player"),
   ...
   (
    ...
    [
        ...
        (try_begin),
          ...
          ## CC 
          # remove enemy heroes from p_total_enemy_casualties
          (party_get_num_companion_stacks, ":num_stacks", "p_total_enemy_casualties"), 
          (try_for_range, ":stack_no", 0, ":num_stacks"),
            (party_stack_get_troop_id, ":stack_troop", "p_total_enemy_casualties", ":stack_no"), 
            (troop_is_hero, ":stack_troop"),
            (party_remove_members, "p_total_enemy_casualties", ":stack_troop", 1),
          (try_end),
          ## CC
...
## CC
  ...
  (
   ...
   (
    ## CC
    "castle_besiege_inner_battle",mnf_scale_picture|mnf_enable_hot_keys, 
    "{s1}^^P, C , I keys are enabled here.",
    ## CC
    "none",
    [
        (troop_get_type, ":is_female", "$g_player_troop"), ## CC
    ...
    [
      ("build_ladders_cont",[],
       "Do it.", [
           ...
           (val_min, ":max_skill", 14),
           (val_div, ":max_skill", 2),
           (store_sub, ":hours_takes", 9, ":max_skill"),
           (try_begin),
             (party_slot_eq, "$current_town", slot_party_type, spt_town),
             (call_script, "script_get_num_ladders"), 
             (assign, ":town_ladders", reg0),
             (val_mul, ":hours_takes", ":town_ladders"),
           (try_end),
           ## CC
    ...
    [
      ("build_siege_tower_cont",[],
       "Start building.", [
           ...
           (assign, ":max_skill", reg0),
           (val_min, ":max_skill", 14),
           (store_sub, ":hours_takes", 18, ":max_skill"),
 ...
 but they offer you all they can find.",
  ...
  (
    ...
    ## CC
      ...
      ("castle_sort_troops_and_auto_recruit_troops", 
        [
          (this_or_next|party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
          (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
          (party_slot_eq, "$g_encountered_party", slot_town_lord, "trp_player"),
        ],
        "Sort the defenders and auto-recruit troops.", 
        [
          (assign, "$g_prsnt_param_1", -1),
    ...
    ## CC
      ...
      ("center_build_fish_pond",[#(eq, reg6, 0),
       ...
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_fish_pond),
                             (jump_to_menu, "mnu_center_improve"),]),
      ("center_build_watch_tower",[#(eq, reg6, 0),
       ...
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_watch_tower),
                               (jump_to_menu, "mnu_center_improve"),]),
      ("center_build_school",[#(eq, reg6, 0),
       ...
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_school),
                          (jump_to_menu, "mnu_center_improve"),]),
      ...
      ("center_build_prisoner_tower",[#(eq, reg6, 0),
       ...
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_prisoner_tower),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ## CC
      ("center_build_barrack",[#(eq, reg6, 0),
                                      ...
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      ## CC
                                      (party_get_slot, ":improvement_level", "$g_encountered_party", slot_center_has_barrack),
                                      (lt, ":improvement_level", 2),
                                      (val_add, ":improvement_level", 1),
                                      (call_script, "script_get_improvement_details", slot_center_has_barrack, "$g_encountered_party", ":improvement_level"),
                                      (str_store_string, s3, s0),
                                      ## CC
                                       ],
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_barrack),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ("center_build_smelter",[#(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_town),
                                      ## CC
                                      (party_get_slot, ":improvement_level", "$g_encountered_party", slot_center_has_smelter),
                                      (lt, ":improvement_level", 2),
                                      (val_add, ":improvement_level", 1),
                                      (call_script, "script_get_improvement_details", slot_center_has_smelter, "$g_encountered_party", ":improvement_level"),
                                      (str_store_string, s3, s0),
                                      ## CC
                                       ],
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_smelter),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ("center_build_training_field",[#(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_castle),
                                      ## CC
                                      (party_get_slot, ":improvement_level", "$g_encountered_party", slot_center_has_training_field),
                                      (lt, ":improvement_level", 2),
                                      (val_add, ":improvement_level", 1),
                                      (call_script, "script_get_improvement_details", slot_center_has_training_field, "$g_encountered_party", ":improvement_level"),
                                      (str_store_string, s3, s0),
                                      ## CC
                                       ],
       "Build a {s3}.",[(assign, "$g_improvement_type", slot_center_has_training_field),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ## CC      
    ...
    [
      ("continue",[], "Continue...",
       [
		  ...
		  #begin of changes
          (party_get_slot, ":bound_town", "$current_town", slot_village_bound_center), ## CC fix
    ...
    [    
      ...
      ## CC
        (try_begin),
          ...
          (ge, ":center_lord", 0),
          (set_fixed_point_multiplier, 100),
          (position_set_x, pos0, 80),
          (position_set_y, pos0, 5),
          (position_set_z, pos0, 60),
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
            ...
            ## CC starting merchant
             ...
             (try_begin),
               ...
               (party_get_slot, ":pt_no", "$current_town", slot_center_mercenary_troop_type),
               (call_script, "script_init_mercenary_template_data", ":pt_no"),
               (party_template_get_slot, ":mercenary_leader", ":pt_no", slot_party_template_leader),
               (set_visitor, ":cur_entry", ":mercenary_leader"),
               (val_add, ":cur_entry", 1),
             (try_end),
             ## CC
			 (try_begin),
...
## CC gambling end
             ...
             (try_end),
			       ## CC
             # add horse_merchants to the tavern
             (try_for_range, ":cur_horse_merchant", horse_merchants_begin, horse_merchants_end),
               (troop_get_slot, ":leaded_party", ":cur_horse_merchant", slot_troop_leaded_party),
               (party_is_in_town, ":leaded_party", "$current_town"),
               (set_visitor, ":cur_entry", ":cur_horse_merchant"),
               (val_add, ":cur_entry", 1),
             (try_end),
             ## CC
...
#           (scene_slot_eq,  ":scene", slot_scene_visited, 1), #check if scene has been visited before to allow entry from menu. Otherwise scene will only be accessible from the town center.
       ...
       [
           ...
           ## CC
            ...
            # trp_temp_troop to record 9 weapon slots from the weaponsmith of town
            (troop_clear_inventory, "trp_temp_troop"),
            (assign, ":fited_weapon_counter", 0),
            (party_get_slot, ":town_weaponsmith", "$current_town", slot_town_weaponsmith),
            (troop_sort_inventory, ":town_weaponsmith"),
            (troop_get_inventory_capacity, ":inv_size", ":town_weaponsmith"),
            (assign, ":end_cond", ":inv_size"),
            (try_for_range, ":i_slot", 10, ":end_cond"),
              (troop_get_inventory_slot, ":item_id", ":town_weaponsmith", ":i_slot"),
              (gt, ":item_id", -1),
              ...
              (val_add, ":fited_weapon_counter", 1),
              # is weapon slot ID, not the weapon in this slot
              (troop_add_item, "trp_temp_troop", ":i_slot", 0),
              (eq, ":fited_weapon_counter", 9),
...
#			(troop_slot_eq, ":lady_guardian", slot_lord_granted_courtship_permission, -1),
	...
	(gt, "$love_interest_in_town", 0),
      ...
      ("trade_with_merchants",
       [
         ...
         "Go to the marketplace (Guild master here).",
         [
           (try_begin),
             (call_script, "script_cf_enter_center_location_bandit_check"),
           (else_try),
    ...
    ## CC
      ...
      [
        ...
        (try_begin),
          (party_slot_eq, "$current_town", slot_town_lord, "trp_player"),
          (party_get_slot, ":cur_improevemnt_data", "$current_town", slot_center_has_barrack),
        ...
        (store_sub, ":time_past", ":hours", ":last_time"),
        
        (try_begin),
          (lt, ":time_past", ":hours_needed"),
          (store_sub, reg1, ":hours_needed", ":time_past"),
          (store_sub, reg2, reg1, 1),
          (str_store_string, s1, "@ ({reg1} {reg2?hours:hour} left)"),
          (disable_menu_option),
        (else_try), 
      ...
      [
        ...
        (val_mul, "$temp_3", ":time_past"),
        (val_div, "$temp_3", ":hours_needed"),
 ...
 In addition to honour, fame and glory, you earn a prize of {reg9} denars. {s8}",
    ...
    [
      ...
      ## CC
        ...
        ## CC
        # give a random weapon from the weaponsmith of town
        (store_random_in_range, ":random_slot", 10, 19), # choose from the 9 weapon slots of trp_temp_troop
        (try_begin),
          (store_free_inventory_capacity, ":free_inv_cap", "trp_player"),
          (gt, ":free_inv_cap", 0), # trp_player have free capacity
          (troop_get_inventory_slot, ":weapon_slot_of_town_weaponsmith", "trp_temp_troop", ":random_slot"),
          ...
          (troop_set_inventory_slot, ":town_weaponsmith", ":weapon_slot_of_town_weaponsmith", -1),
          (troop_sort_inventory, ":town_weaponsmith"),
          (call_script, "script_sort_merchant_inventory_by_type", ":town_weaponsmith"),
          # message
          ...
          (display_message, "@The local weaponsmith give you a weapon for your victory.", 0x44FF44),
          (display_message, "@You get an item: {s2}{s1}.", 0x44FF44),
        (try_end),
        (troop_clear_inventory, "trp_temp_troop"),
        ## CC
        ],
  ...
  (
    ...
    [
      ...
      ("tournament_join_next_fight", [], "Fight in the next round.",
       [
           ...
           (try_begin),
             ...
             #Swadia
             (store_mod, ":mod", ":town_index_within_faction", 6), ## CC
             (try_begin),
               (eq, ":mod", 0), # "town_4","Suno"
               (call_script, "script_set_items_for_tournament", 40, 80, 50, 20, 0, 0, 0, 60, 0), ## CC
             (else_try),
               (eq, ":mod", 1), # "town_6","Praven"
               (call_script, "script_set_items_for_tournament", 100, 100, 0, 0, 0, 0, 0, 0, 0),
             (else_try),
               (eq, ":mod", 2), # "town_7","Uxkhal"
               (call_script, "script_set_items_for_tournament", 100, 0, 100, 0, 0, 0, 0, 0, 0),
             (else_try),
               (eq, ":mod", 3), # "town_16","Dhirim"
               (call_script, "script_set_items_for_tournament", 60, 50, 80, 20, 0, 0, 0, 40, 0), ## CC
             ## CC
             (else_try),
               (eq, ":mod", 4), # "town_23","Keebur"
               (call_script, "script_set_items_for_tournament", 0, 0, 0, 0, 0, 0, 0, 0, 100),
             (else_try),
               (eq, ":mod", 5), # "town_24","Betazim"
               (call_script, "script_set_items_for_tournament", 30, 30, 50, 0, 0, 0, 0, 40, 40),
             ## CC
             (try_end),
           (else_try),
             (eq, ":town_original_faction", "fac_kingdom_2"),
             #Vaegirs
             (store_mod, ":mod", ":town_index_within_faction", 5), ## CC
             (try_begin),
               (eq, ":mod", 0), # "town_8","Reyvadin"
               #(call_script, "script_set_items_for_tournament", 40, 80, 50, 20, 0, 0, 0, 0, 0),
               (call_script, "script_set_items_for_tournament", 0, 0, 0, 0, 0, 100, 0, 0, 0),
             (else_try),
               (eq, ":mod", 1), # "town_9","Khudan"
               (call_script, "script_set_items_for_tournament", 100, 50, 0, 0, 0, 0, 20, 30, 0),
             (else_try),
               (eq, ":mod", 2), # "town_11","Curaw"
               (call_script, "script_set_items_for_tournament", 100, 0, 50, 0, 0, 0, 20, 30, 0),
             (else_try),
               (eq, ":mod", 3), # "town_13","Rivacheg"
               (call_script, "script_set_items_for_tournament", 40, 80, 50, 20, 0, 30, 0, 60, 0),
             ## CC
             (else_try),
               (eq, ":mod", 4), # "town_25","Itice"
               #(call_script, "script_set_items_for_tournament", 0, 0, 0, 0, 0, 100, 0, 0, 0),
               (call_script, "script_set_items_for_tournament", 40, 80, 50, 20, 0, 0, 0, 0, 0),
             ## CC
             (try_end),
           (else_try),
             ...
             (store_mod, ":mod", ":town_index_within_faction", 5), ## CC
             (try_begin),
               (eq, ":mod", 0), # "town_10","Tulga"
               (call_script, "script_set_items_for_tournament", 100, 0, 0, 0, 0, 0, 0, 100, 0),
             (else_try),
               (eq, ":mod", 1), # "town_14","Halmar"
               (call_script, "script_set_items_for_tournament", 100, 50, 25, 0, 25, 0, 30, 50, 0),
             ## CC
             (else_try),
               (eq, ":mod", 2), # "town_17","Ichamur"
               (call_script, "script_set_items_for_tournament", 100, 100, 0, 0, 0, 0, 0, 0, 0),
             (else_try),
               (eq, ":mod", 3), # "town_18","Narra"
               (call_script, "script_set_items_for_tournament", 100, 50, 30, 25, 0, 0, 0, 40, 0),
             (else_try),
               (eq, ":mod", 4), # "town_26","Hottam"
               (call_script, "script_set_items_for_tournament", 100, 0, 0, 0, 0, 0, 100, 0, 0),
             ## CC
             (try_end),
           (else_try),
             (eq, ":town_original_faction", "fac_kingdom_4"),
             #Nords
             (store_mod, ":mod", ":town_index_within_faction", 4), ## CC
             (try_begin),
               (eq, ":mod", 0), # "town_1","Sargoth"
               (call_script, "script_set_items_for_tournament", 0, 0, 50, 80, 0, 0, 0, 0, 0),
             (else_try),
               (eq, ":mod", 1), # "town_2","Tihr"
               #(call_script, "script_set_items_for_tournament", 0, 0, 50, 80, 0, 50, 0, 0, 0),
               (call_script, "script_set_items_for_tournament", 40, 0, 0, 60, 0, 50, 80, 0, 0),
             (else_try),
               (eq, ":mod", 2), # "town_12","Wercheg"
               (call_script, "script_set_items_for_tournament", 40, 0, 0, 100, 0, 0, 0, 0, 0),
             ## CC
             (else_try),
               (eq, ":mod", 3), # "town_27","Lathow Bay"
               #(call_script, "script_set_items_for_tournament", 40, 0, 0, 60, 0, 50, 80, 0, 0),
               (call_script, "script_set_items_for_tournament", 0, 0, 50, 80, 0, 50, 0, 0, 0),
             ## CC
             (try_end),
           (else_try),
             ...
             (store_mod, ":mod", ":town_index_within_faction", 4), ## CC 
             (try_begin),
               (eq, ":mod", 0), # "town_3","Veluca"
               #(call_script, "script_set_items_for_tournament", 25, 30, 60, 0, 0, 0, 0, 0, 50),
               (call_script, "script_set_items_for_tournament", 100, 100, 0, 0, 0, 0, 0, 0, 0),
             (else_try),
               (eq, ":mod", 1), # "town_5","Jelkala"
               (call_script, "script_set_items_for_tournament", 25, 30, 30, 0, 50, 0, 0, 40, 30),
             (else_try),
               (eq, ":mod", 2), # "town_15","Yalen"
               (call_script, "script_set_items_for_tournament", 0, 0, 60, 0, 60, 0, 0, 0, 30),
             (else_try),
               (eq, ":mod", 3), # "town_28","Subtech"
               #(call_script, "script_set_items_for_tournament", 100, 100, 0, 0, 0, 0, 0, 0, 0),
               (call_script, "script_set_items_for_tournament", 25, 30, 60, 0, 0, 0, 0, 20, 50),
             (try_end),
             ## CC
           (else_try),
             #Sarranids
             ...
             (store_mod, ":mod", ":town_index_within_faction", 5), ## CC
             (try_begin),
               (eq, ":mod", 0), # "town_19","Shariz"
               (call_script, "script_set_items_for_tournament", 100, 40, 60, 0, 0, 30, 30, 0, 0),
             (else_try),
               (eq, ":mod", 1), # "town_20","Durquba"
               (call_script, "script_set_items_for_tournament", 50, 0, 30, 30, 60, 0, 0, 0, 0),
             (else_try),
               (eq, ":mod", 2), # "town_21","Ahmerrad"
               (call_script, "script_set_items_for_tournament", 80, 0, 60, 0, 50, 0, 50, 0, 0),
             (else_try),
               (eq, ":mod", 3), # "town_22","Bariyye"
               (call_script, "script_set_items_for_tournament", 50, 80, 40, 0, 60, 0, 60, 0, 0),
             (else_try),
               (eq, ":mod", 4), # "town_29","Launocane"
               (call_script, "script_set_items_for_tournament", 50, 0, 0, 0, 60, 0, 60, 0, 0),
             (try_end),
             ## CC
           (try_end),
           (jump_to_scene, ":arena_scene"),
      ...
      ## CC
      ("tournament_view_prizes", [], "View prizes.",
       [(start_presentation, "prsnt_tournament_prizes"),
        ]),
      ## CC
  ...
  (
    ...
    "As the party member with the highest speechcraft skill ({reg2}), {reg3?you expect:{s1} expects} that collecting taxes from here will take {reg4} days...",
    "none",
...
##	(call_script, "script_merchant_road_info_to_s42", "$g_encountered_party"),
  ...
  (
    ...
    "As the party member with the highest speechcraft skill ({reg2}), {reg3?you try to figure out:{s1} tries to figure out} the best goods to trade in. {s2}",
    "none",
    [
        ...
        (try_begin), # rebellion failed
          ...
          (try_begin),
            ...
            (try_begin),
              ...
              (party_set_marshal, ":rebel_faction_marshall_party", 0),
            (try_end),
            # lords & parties
...
#    ]),
  ...
  ("choose_options_1", 0,
    ...
    [
      ("yes_npcs", [], "Yes",
        [
          (assign, "$g_gather_npcs", 1),
          (jump_to_menu,"mnu_choose_options_2"),
        ]
      ),
      ("no_npcs", [], "No",
        [
          (assign, "$g_gather_npcs", 0),
          (jump_to_menu,"mnu_choose_options_2"),
        ]
      ),
      ("go_back", [], "Go back",
...
## CC
  ...
  (
    ...
    [
      ("map_precision_1",[],"Very Low",
       [
        (assign, "$g_prsnt_param_1", 6),
        (start_presentation, "prsnt_world_map"),
       ]
      ),
      ("map_precision_2",[],"Low.",
       [
        (assign, "$g_prsnt_param_1", 5),
        (start_presentation, "prsnt_world_map"),
       ]
      ),
      ("map_precision_3",[],"Medium.",
       [
        (assign, "$g_prsnt_param_1", 4),
        (start_presentation, "prsnt_world_map"),
      ]
       ),
      ("map_precision_4",[],"High.",
       [
        (assign, "$g_prsnt_param_1", 3),
        (start_presentation, "prsnt_world_map"),
      ]
       ),
      ("map_precision_5",[],"Ultra High.",
       [
        (assign, "$g_prsnt_param_1", 2),
        (start_presentation, "prsnt_world_map"),
      ]
       ),
      ("go_back",[],"Go back.",
  ...
  (
    "battlefield_size",0,
    "Please choose the size of battlefield.",
    "none", [],
    [
      ("battlefield_small",[],"Small.", 
        [(assign, "$g_random_scene_size", 0),
        (jump_to_menu,"$g_next_menu"),
        ]),
      ("battlefield_normal",[],"Normal.", 
        [(assign, "$g_random_scene_size", 1),
        (jump_to_menu,"$g_next_menu"),
        ]),
      ("battlefield_large",[],"Large.", 
        [(assign, "$g_random_scene_size", 2),
        (jump_to_menu,"$g_next_menu"),
        ]),
  ...
  (
    ...
    [
      ...
      (party_get_free_companions_capacity, ":free_capacity", "p_main_party"),
      (try_begin), 
        (this_or_next|le, ":free_capacity", 0), # not enough room
        (this_or_next|le, "$temp_3", 0), # recruit chances is out
        (lt, ":gold", reinforcement_cost_easy), # not enough money
        (party_clear, "p_temp_party"),
        (jump_to_menu, "mnu_town"),
      (try_end),
    ],
    [
      ("recruit_to_your_party",[],"Recruit them to your party.",
       [
        (store_troop_gold, ":gold", "trp_player"),
        (ge, ":gold", reinforcement_cost_easy),
        (troop_remove_gold, "trp_player", reinforcement_cost_easy),
        # (try_begin),
